...
*orientationType* | | string | Enum {'GLOBAL', 'TANGENTIAL'}: <br>'GLOBAL': relative to the global project-specific map coordinate system, only valid for omniodirectional vehicles;<br>'TANGENTIAL': tangential to the edge, any value valid for omnidirectional, differential vehicles only 0.0 (forward) and Pi (backward) possible.<br><br>If not defined, the default value is 'TANGENTIAL'.
