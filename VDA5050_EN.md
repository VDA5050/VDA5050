...
localized | | boolean | "true": position is initialized. `x`, `y` and `theta` can be trusted.<br>"false": position is not initialized. `x`, `y` and `theta` cannot be trusted.<br>Changing to the state to "false" is only allowed if the vehicle cannot determine its position anymore. The vehicle shall not clear its order when localization is lost. The vehicle shall report this state via an error (`errorType` = "localizationError", `errorLevel` = "FATAL").<br>If `localizationScore` is provided the value should be set to zero. While this is set to false, the vehicle shall not resume driving or continue its order.
