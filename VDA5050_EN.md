...
positionInitialized | | boolean | "true": position is initialized. `x`, `y` and `theta` can be trusted.<br>"false": position is not initialized. `x`, `y` and `theta` cannot be trusted.<br>Changing to the state to "false" is only allowed if the vehicle cannot determine its position anymore and the vehicle is not driving (`driving=false`). When entering this state, the vehicle immediately clears the current order. The vehicle should additionally report this state via an error (`errorType` = "FATAL").<br>If `localizationScore` is provided the value should be set to zero.
