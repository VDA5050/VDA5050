...
Potential conflicts in orders due to overlapping of zones or combination of zone and edge properties and how to resolve them are addressed in section [6.4.4 Interaction between zones](#644-interactions-between-zones). For nodes that are part of the order but are in conflict with zones (e.g., released node within a 'RELEASE' zone), the robot is expected to act according to the zones (e.g., wait for RELEASE state of the request).
