...
*releaseLossBehavior* <br> } | | string | Enum { 'STOP' , 'RETURN' }<br>Defines how the robot shall behave in the case of either its release of a corridor expiring or the release being revoked by the fleet control.<br>'STOP': Mobile robot stops in its current position and awaits manual intervention. It keeps the request in its state with the `requestStatus` set to 'REVOKED' or 'EXPIRED' accordingly and reports a 'RELEASE_LOST' error with level 'CRITICAL'.<br>'RETURN': Mobile robot returns to the predefined trajectory of the edge it deviated from, along the path it took to deviate from it. It keeps the request in its state with the `requestStatus` set to 'REVOKED' or 'EXPIRED' accordingly and reports a 'RELEASE_LOSS_HANDLING' error with level 'WARNING' until it has reached the predefined trajectory.<br>Default: 'STOP'.
