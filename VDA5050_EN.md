...
startHibernate | stopHibernate              | Initiates hibernate mode, in which the AGV remains connected to the MQTT-broker but stops sending state messages. <br>Before entering, the AGV must publish the final connection state HIBERNATE. <br>The AGV shall be able to receive and respond to the instantAction stopHibernate while it is in HIBERNATE connection state or controllers can subscribe the instantAction stopHibernate and translate it into AGV-specific commands to exit hibernate mode. <br>No further driving movements or action execution occur; reaching the next node is not required. If the AGV battery becomes critically low during hibernate mode, the AGV may exit hibernate autonomously to report an error.<br>If specified in the factsheet, the order is resumeable, else order must be resent once the AGV returns to ONLINE.                                                                                                                                                                                                   | yes | - | - | yes | no | no
stopHibernate | startHibernate | Deactivates hibernate mode. <br>On success, the AGV sets the connection state to ONLINE.<br>Movement and actions are resumed once the order is retained by the AGV or it was resent by the MC.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | yes | - | - | yes | no | no
shutdown | - | Initiates a coordinated shutdown, in which the AGV disconnects from the MQTT broker and clears all orders. There is no VDA5050-compliant way to automatically return to ONLINE because the connection is terminated. <br>If an AGV is in hibernate mode and needs to be shut down, it must first exit hibernate (stopHibernate) before executing shutdown.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | yes | - | - | yes | no | no                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | yes | -                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | paused | yes | no | no
...
startHibernate | - | Activation of the HIBERNATE connection state is in preparation. If the AGV supports an instant transition, this state can be omitted.| - | Vehicle stands still. All actions are paused. No state messages are sent by the AGV.<br>Hibernate mode is activated. The AGV reports connectionState "HIBERNATE".| The HIBERNATE connection state could not be activated (e.g., overridden by a hardware switch).
stopHibernate | - | Deactivation of the HIBERNATE connection state is in preparation. If the AGV supports an instant transition, this state can be omitted.| - | Hibernate mode is deactivated. Movement and all other actions will resume if the order is retained or once the order is resent.<br>The AGV reports connectionState "ONLINE".| The HIBERNATE connection state could not be deactivated (e.g., overridden by a hardware switch).
shutdown | - | Activation of the OFFLINE connection state is in preparation. If the AGV supports an instant transition, this state can be omitted.| - |Vehicle stands still. All actions are paused, and all orders are cleared. The connection between AGV and broker is terminated in a coordinated way.<br>The AGV reports connectionState "OFFLINE".| The OFFLINE connection state could not be activated (e.g., overridden by a hardware switch).
...
manufacturer | string | Manufacturer of the AGV.
serialNumber | string | Serial number of the AGV.
connectionState | string | Enum {'ONLINE', 'OFFLINE', 'CONNECTIONBROKEN'}<br><br>'ONLINE': connection between AGV and broker is active.<br><br>'OFFLINE': connection between AGV and broker has gone offline in a coordinated way. <br><br>'HIBERNATE': connection between AGV and broker is active, but AGV stops sending state messages. This mode is intended for power-saving or communication reduction. The AGV can later resume to ONLINE when instructed.<br><br>'CONNECTIONBROKEN': the connection between AGV and broker has unexpectedly ended.
...
| **Field**                                          | **data type**        | **description**                                                                                                                                                                                                                                                                                                                   |
|----------------------------------------------------|----------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ***orderHandling*** [***orderHandling***]          | array of JSON object | Defines the vehicle’s behavior and capabilities related to order processing, including special conditions such as hibernate connection state.                                                                                                                                                                                                                                                                                                                                  |
| {                                                  
| &emsp;*keepOrderOnHibernate*                       | boolean | Indicates whether the vehicle retains its current order when entering the HIBERNATE connection state. If true, the AGV will keep the active order and will resume execution after returning to ONLINE.
| }                                                  
