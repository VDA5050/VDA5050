...
주행 이외의 동작을 모바일 로봇이 지원하는 경우, 해당 동작은 노드나 edge에 붙은 `actions` 배열로 지시하거나, 별도 토픽 `instantActions`로 보냅니다([6.2.1 Instant actions](#621-instant-actions)). 또는 액션 Zone으로 구성합니다([6.4.1 Zone 유형](/spec/6/6.4#641-zone-types)).
edge에서 실행되는 동작은 모바일 로봇이 그 edge 위에 있는 동안에만 실행되어야 합니다([6.6.2 노드·edge 통과 및 동작 트리거](/spec/6/6.6#662-traversal-of-nodes-and-enteringleaving-edges-triggering-of-actions)).
...
그림 11은 모바일 로봇이 동작의 blocking 유형을 처리하는 방법을 설명합니다. 모바일 로봇이 새 동작을 실행할 지점(노드, edge, 액션 Zone)에 도착하면 `actions` 배열과 같은 순서로 대기열에 넣습니다. 이 대기열은 그림 11과 같이 계속 처리됩니다. 대기열의 어떤 동작이든 blocking 유형이 'SOFT' 또는 'HARD'이면 자동 주행을 멈춰야 합니다. 동작의 blocking 유형이 'NONE' 또는 'SOFT'이면 병렬 실행을 위해 모읍니다. blocking 유형이 'SINGLE' 또는 'HARD'인 동작을 실행하려면 수집된 병렬 동작이 모두 'FINISHED' 또는 'FAILED'여야 합니다. 대기열에 더 이상 'SOFT' 또는 'HARD'가 없으면 자동 주행을 재개할 수 있습니다. 'FINISHED' 또는 'FAILED' 동작은 대기열에서 제거합니다.
