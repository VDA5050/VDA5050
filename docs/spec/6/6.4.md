...
Order에서 Zone과 edge 속성의 겹침·조합으로 인한 잠재적 충돌과 해결 방법은 [6.4.4 Zone 상호작용](#644-interactions-between-zones)에 다룹니다. Order의 일부로 릴리스되었으나 Zone으로 제한된 노드(예: 'BLOCKED' 또는 'RELEASE' Zone 안의 노드)에 대해서는 로봇이 Zone에 따라 동작해야 합니다(예: 진입하지 않거나 'GRANTED'까지 대기).
...
| LINE_GUIDED | none | | 이 Zone에서는 자유 내비게이션이 허용되지 않으며, 모바일 로봇은 edge의 미리 정의된 궤적을 따라야 합니다. 경로가 노드-edge 그래프 형태로 플릿 제어에 명시된 경우에만 이 Zone에 들어갈 수 있습니다. Zone 진입에 윤곽 밖 이동이 필요하면 미리 정의된 궤적을 따라야 합니다. 진입 시 모바일 로봇은 Zone을 가로지르는 edge의 궤적 위에 있어야 합니다. Zone 안으로 들어오고 Zone 내부에 있는 edge는 플릿 제어가 보낸 궤적 또는 모바일 로봇에 미리 정의된 궤적이 필요합니다. 코리도를 보내 궤적에서 벗어나도록 할 수 있습니다. | 
